...
export TERRAFORM_PROVIDER_REPO ?= https://github.com/hashicorp/terraform-provider-vsphere.git
export TERRAFORM_PROVIDER_VERSION ?= 2.3.1
export TERRAFORM_PROVIDER_DOWNLOAD_NAME ?= terraform-provider-vsphere
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-vsphere_2.3.1_linux_amd64
...
xpkg.build.provider-vsphere: do.build.images
...
pull-docs:
	@if [ ! -d "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)" ]; then \
		...
		git clone -c advice.detachedHead=false --depth 1 --branch "v$(TERRAFORM_PROVIDER_VERSION)" "$(TERRAFORM_PROVIDER_REPO)" "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)"; \
	fi
